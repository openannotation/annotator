(function() {
  ...
  Annotator.Plugins.Store = (function() {
    ...
    function Store(element, options) {
      var auth, callback, getAnnotations, s;
      ...
      getAnnotations = __bind(function() {
        if (this.options.loadFromSearch) {
          return this.loadAnnotationsFromSearch(this.options.loadFromSearch, callback);
        } else {
          return this.loadAnnotations(callback);
        }
      }, this);
    ...
    Store.prototype.loadAnnotations = function(callback) {
      return this._apiRequest('read', null, __bind(function(data) {
        this.annotations = data.slice();
        return this.annotator.loadAnnotations(data, callback);
      }, this));
    };
    Store.prototype.loadAnnotationsFromSearch = function(searchOptions, callback) {
      return this._apiRequest('search', searchOptions, __bind(function(data) {
        this.annotations = data.results.slice();
        return this.annotator.loadAnnotations(data.results, callback);
      }, this));
    };
    Store.prototype.dumpAnnotations = function() {
      return _.map(this.annotations, __bind(function(ann) {
        return JSON.parse(this._dataFor(ann));
      }, this));
    };
    Store.prototype._apiRequest = function(action, obj, onSuccess) {
      var onBeforeSend, onError, opts;
      onBeforeSend = __bind(function(xhr) {
        var headers, key, val, _len, _ref, _results;
