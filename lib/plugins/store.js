...
Annotator.Plugins.Store = DelegatorClass.extend({
  ...
  init: function (options, element) {
    ...
    var getAnnotations = function () {
      if (self.options.loadFromSearch) {
        self.loadAnnotationsFromSearch(self.options.loadFromSearch, callback)
      } else {
        self.loadAnnotations(callback)
      }
    }

    var auth = $(this.element).data('annotator:auth')
    if (auth) {
  ...
  _urlFor: function (action, id) {
    var replaceWith = (id === null || typeof(id) === 'undefined') ? '' : '/' + id
