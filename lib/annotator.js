...
this.Annotator = DelegatorClass.extend({
    ...
    init: function (options, element) {
        ...
        var annotator = this;
        $.each(['adder', 'creater', 'highlighter'], function (idx, name) {
    ...
    register: function (annotation, suppressEvents) {
        var annotator = this;
    ...
    clearAll: function () {
        this.annotations = [];
        $('.' + this.options.classPrefix + '-highlighter').each(function () {
...
this.AnnotationStore = DelegatorClass.extend({
    ...
    annotationCreated: function (e, annotator, newAnnotation) {
        var self = this;
        $.ajax({
            url: this._urlFor('create'),
            data: $.param(newAnnotation),
            dataType: 'jsonp',
            type: 'PUT',
            success: function (data) {
                self.registerAnnotation(data);
            }
        });
    },

    registerAnnotation: function (annotationData) {
    ...
    loadAnnotation: function (id) {
        ...
        $.getJSON(this._urlFor('read', id), null, function (data, textStatus) {
            if (textStatus === 'success') {
                self.options.annotator.loadAnnotations(id ? [data] : data);
            } else {
    ...
    _urlFor: function (action, id) {
        ...
        return url + this.options.urls[action].replace(/:id/, id || 'all');
    }
