...
Annotator.Plugins.Auth = DelegatorClass.extend({
  ...
  setToken: function (token) {
    ...
    if (this.haveValidToken()) {
      ...
      // Prevent over-fast token requests
      if (beforeExpiry < 0) {
        console.warn("Token TTL too short, or token already expired. Getting a new token in 10s.")
        beforeExpiry = 10
        dudToken = true
      }
      ...
      setTimeout(function () { self.requestToken() }, 10 * 1000)
    }
  },
