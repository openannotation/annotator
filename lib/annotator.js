(function() {
  ...
  Annotator = (function() {
    ...
    Annotator.prototype.highlightRange = function(normedRange) {
      var elemList, node, textNodes, wrapper;
      textNodes = $(normedRange.commonAncestor).textNodes();
      textNodes = textNodes.slice(textNodes.index(normedRange.start), textNodes.index(normedRange.end) + 1);
    ...
    Annotator.prototype.addPlugin = function(name, options) {
      ...
      if (typeof klass === 'function') {
        return new klass(this.element, options);
      } else {
        return alert("Could not load " + name + " plugin. Have you included the appropriate <script> tag?");
      }
    };
    ...
    Annotator.prototype.showViewer = function(e, annotations) {
      ...
      for (_i = 0, _len = annotations.length; _i < _len; _i++) {
        ...
        $("<div><div class='" + this.options.classPrefix + "-text'><p>" + annot.text + "</p></div>" + controlsHTML + "</div>").appendTo(viewerclone).data("annotation", annot);
      }
